...
// Market state
let currentPrice = new Decimal(150.1); // Starting SOL price
...
const fillUserWalletForTestAssets = async (
  jwt: string,
  baseAmount: number,
  quoteAmount: number,
) => {
...
const updateMarketPrice = () => {
  ...
  const meanReversion = new Decimal(150)
    .sub(currentPrice)
    .mul(meanReversionStrength);
