...
class Orderbook {
  ...
  constructor() {
    this.bids = new BTree<Decimal, OrderQueue>(
      undefined,
      (a: Decimal, b: Decimal) => b.comparedTo(a),
    );

    this.asks = new BTree<Decimal, OrderQueue>(
      undefined,
    ...
    for (const level of snapshot.bids) {
      ...
      for (const rawOrder of level.orders) {
        const restoredOrder = {
          ...
          quoteAmount: rawOrder.quoteAmount
            ? new Decimal(rawOrder.quoteAmount)
            : null,
          quoteRemaining: rawOrder.quoteRemaining
            ? new Decimal(rawOrder.quoteRemaining)
            : null,
          quoteSpent: rawOrder.quoteSpent
            ? new Decimal(rawOrder.quoteSpent)
            : null,
        };

        queue.enqueue(restoredOrder);
        this.orderIndex.set(restoredOrder.id, {
          side: "BUY",
          price: priceDecimal,
        });
      }
